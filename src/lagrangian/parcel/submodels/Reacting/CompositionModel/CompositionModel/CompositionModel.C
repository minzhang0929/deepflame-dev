...
:
    ...
    carrierMixture_
    (
        isA<CanteraSpecies>(carrierThermo_)
      ? &refCast<const CanteraSpecies>(carrierThermo_)
      : nullptr
    ),
...
) const
{
    ...
    {
        Info<<"in CompositionModel species: "<<carrierMixture_->species()[i]<<endl;

...
)
{
    ...
    if (cloud.heatTransfer().BirdCorrection())
    {
        ...
        composition.carrier().calcCp(td.pc(), Tsdash);
        forAll(dMassPC, i)
        {
            const label cid = composition.localToCarrierId(idPhase, i);
...
)
{
    ...
    scalar CpEff = 0.0;
    cloud.composition().carrier().calcCp(td.pc(), td.Tc());
    forAll(cloud.rhoTrans(), i)
    {
...
)
{
    ...
    const CanteraSpecies& carrier = cloud.composition().carrier();
...
)
{
    ...
    if (np0*mass1 < cloud.constProps().minParcelMass())
    {
        ...
        if (cloud.solution().coupled())
        {
            ...
            composition.carrier().calcH(td.pc(), T0);
            forAll(Y_, i)
            {
                scalar dmi = dm*Y_[i];

...
Foam::wordList Foam::fv::clouds::addSupFields() const
{
    ...
    if (carrierHasThermo_)
    {
        ...
        if (isA<CanteraSpecies>(carrierThermo))
        {
...
) const
{
    ...
    (
        isA<CanteraSpecies>(carrierThermo)
     && refCast<const CanteraSpecies>(carrierThermo).contains
        (
            eqn.psi().name()
        )
    )
    {
        eqn += cloudsPtr_().SYi
        (
            refCast<const CanteraSpecies>(carrierThermo).index(eqn.psi()),
            eqn.psi()
        );
    }
}

...
:
    ...
    curTimeIndex_(-1)
{
    if (isA<CanteraSpecies>(surfaceFilm_.primaryThermo()))
    {
        const CanteraSpecies& composition =
            refCast<const CanteraSpecies>(surfaceFilm_.primaryThermo());
...
) const
{
    ...
    (
        isA<CanteraSpecies>(surfaceFilm_.primaryThermo())
     && refCast<const CanteraSpecies>(surfaceFilm_.primaryThermo()).contains
        (
            eqn.psi().name()
        )
    )
